...
impl Protorune {
    pub fn index_runestone<T: MessageContext>(
        ...
        for (vout, sheet) in balances_by_output.clone() {
            ...
            sheet.save(&mut atomic.derive(&table.OUTPOINT_TO_RUNES.select(&outpoint_bytes)), false);
            ...
            for (rune, amount) in sheet.balances.iter() {
                if *amount > 0 {
                    ...
                    atomic
                        .derive(
                            &table.RUNE_OUTPOINT_MAPPING.select(
    ...
    pub fn add_rune_outpoint_mapping(
        atomic: &mut AtomicPointer,
        rune_id: &ProtoruneRuneId,
        outpoint: &OutPoint
    ) -> Result<()> {
        let rune_id_key = rune_id.to_string();
        let outpoint_bytes = consensus_encode(outpoint)?;

        atomic
            .derive(&tables::RUNES.RUNE_OUTPOINT_MAPPING.select(&rune_id_key.into_bytes()))
            .append(Arc::new(outpoint_bytes));
    ...
    pub fn update_balances_for_edict(
        ...
        let sheet = balances_by_output
            ...
            .ok_or(anyhow!("balance sheet not present"))?;

        let old_amount = balance_sheet.get(&(*rune_id).into());
        let amount = if edict_amount == 0 {
            old_amount
        } else {
            std::cmp::min(edict_amount, old_amount)
        };
        ...
        if amount > 0 {
            ...
            Self::add_rune_outpoint_mapping(atomic, rune_id, &new_outpoint)?;
        }
    ...
    pub fn process_edicts(
        ...
        for edict in edicts {
            Self::process_edict(atomic, tx, edict, balances_by_output, balances, outs, &edict.id)?;
        }
        Ok(())
    }
    ...
    fn create_compound_key(rune: &ProtoruneRuneId, outpoint: &OutPoint) -> Result<String> {
        let rune_id_str = rune.to_string();
        let outpoint_bytes = consensus_encode(outpoint)?;
        let outpoint_hex = hex::encode(&outpoint_bytes);

        Ok(format!("{}:{}", rune_id_str, outpoint_hex))
    }
    ...
    pub fn remove_rune_outpoint_mapping(
        ...
        empty_sheet.save(
            &mut atomic.derive(&tables::RUNES.OUTPOINT_TO_RUNES.select(&outpoint_bytes)),
            true
        );
            ...
            if let Some(Artifact::Runestone(ref runestone)) = Runestone::decipher(tx) {
                ...
                for input in &tx.input {
                    ...
                    if !previous_output.is_null() {
                        Self::remove_rune_outpoint_mapping(&mut atomic, previous_output)?;
                    }
                }
                match

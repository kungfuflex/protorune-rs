...
impl Protorune {
    ...
    pub fn index_mint(mint: &ProtoruneRuneId, height: u64, balance_sheet: &mut BalanceSheet) -> Result<()> {
    ...
    pub fn build_rune_id(height: u64, tx: u32) -> Arc<Vec<u8>> {
        let rune_id = ProtoruneRuneId::new(height as u128, tx as u128).to_string().into_bytes();

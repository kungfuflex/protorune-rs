...
impl Protorune {
    ...
    pub fn process_edict(
        ...
        if edict.id.block == 0 && edict.id.tx != 0 { Err(anyhow!("invalid edict")) } else { Ok(()) }
    }
    ...
    pub fn index_etching(
        ...
        if let Some(name) = etching.rune {
            ...
            tables::RUNES.RUNE_ID_TO_ETCHING
                .select(&rune_id.clone())
                .set(Arc::new(name.0.to_string().into_bytes()));
            tables::RUNES.ETCHING_TO_RUNE_ID
                .select(&name.0.to_string().into_bytes())
                .set(rune_id.clone());
            tables::RUNES.RUNE_ID_TO_HEIGHT.select(&rune_id.clone()).set_value(height);

            if let Some(divisibility) = etching.divisibility {
                tables::RUNES.DIVISIBILITY
                    .select(&name.0.to_string().into_bytes())
            ...
            if let Some(terms) = etching.terms {
                if let Some(amount) = terms.amount {
                    tables::RUNES.AMOUNT.select(&name.0.to_string().into_bytes()).set_value(amount);
                }
            ...
            if let Some(spacers) = etching.spacers {
                tables::RUNES.SYMBOL.select(&name.0.to_string().into_bytes()).set_value(spacers);
            }

            tables::RUNES.ETCHINGS
                .select(&name.0.to_string().into_bytes())
